...
class LeagueBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        await db_exec(self.db, "CREATE TABLE IF NOT EXISTS leagues (id INTEGER PRIMARY KEY AUTOINCREMENT, guild_id TEXT, name TEXT, status TEXT);")
        await db_exec(self.db, "CREATE TABLE IF NOT EXISTS players (league_id INTEGER, user_id TEXT, PRIMARY KEY (league_id, user_id));")
        await db_exec(self.db, "CREATE TABLE IF NOT EXISTS matches (id INTEGER PRIMARY KEY AUTOINCREMENT, league_id INTEGER, p1 TEXT, p2 TEXT, result TEXT, created_at INTEGER);")
        await db_exec(self.db, "CREATE TABLE IF NOT EXISTS standings (league_id INTEGER, user_id TEXT, wins INTEGER DEFAULT 0, draws INTEGER DEFAULT 0, losses INTEGER DEFAULT 0, points INTEGER DEFAULT 0, PRIMARY KEY (league_id, user_id));")
...
async def get_open_league(guild_id: int):
    return await db_one(bot.db, "SELECT * FROM leagues WHERE guild_id=? AND status='open' ORDER BY id DESC LIMIT 1", (str(guild_id),))
...
async def league_create(interaction: discord.Interaction, name: str):
    await db_exec(bot.db, "INSERT INTO leagues (guild_id, name, status) VALUES (?,?,?)", (str(interaction.guild_id), name, "open"))
...
async def joinleague(interaction: discord.Interaction):
    ...
    await db_exec(bot.db, "INSERT OR IGNORE INTO players (league_id, user_id) VALUES (?,?)", (league["id"], str(interaction.user.id)))
...
async def winversus(interaction: discord.Interaction, adversaire: discord.User):
    ...
    await db_exec(bot.db, "INSERT INTO matches (league_id, p1, p2, result, created_at) VALUES (?,?,?,?,?)",
                  (league["id"], str(interaction.user.id), str(adversaire.id), "win", now()))

    await db_exec(bot.db, "UPDATE standings SET wins=wins+1, points=points+3 WHERE league_id=? AND user_id=?",
                  (league["id"], str(interaction.user.id)))
    await db_exec(bot.db, "UPDATE standings SET losses=losses+1 WHERE league_id=? AND user_id=?",
                  (league["id"], str(adversaire.id)))

    await interaction.response.send_message(f"🏆 <@{interaction.user.id}> gagne contre <@{adversaire.id}> (+3 pts)")
...
async def drawversus(interaction: discord.Interaction, adversaire: discord.User):
    ...
    for uid in (interaction.user.id, adversaire.id):
        ...
        await db_exec(bot.db, "UPDATE standings SET draws=draws+1, points=points+1 WHERE league_id=? AND user_id=?",
                      (league["id"], str(uid)))

    await db_exec(bot.db, "INSERT INTO matches (league_id, p1, p2, result, created_at) VALUES (?,?,?,?,?)",
                  (league["id"], str(interaction.user.id), str(adversaire.id), "draw", now()))

    await interaction.response.send_message(f"🤝 Égalité entre <@{interaction.user.id}> et <@{adversaire.id}> (+1 pt chacun)")
...
async def league_leaderboard(interaction: discord.Interaction):
    ...
    rows = await db_all(bot.db, "SELECT * FROM standings WHERE league_id=? ORDER BY points DESC", (league["id"],))
    if not rows:
