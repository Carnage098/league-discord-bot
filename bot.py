...
async def is_player(league_id: int, user_id: str) -> bool:
    row = await db_one(
        bot.db,
        "SELECT 1 FROM players WHERE league_id=? AND user_id=?",
        (league_id, user_id),
    )
    return row is not None


async def require_both_registered(
    interaction: discord.Interaction, league_id: int, opponent: discord.User
) -> bool:
    if opponent.bot:
        await interaction.response.send_message(
            "❌ Tu ne peux pas jouer contre un bot.", ephemeral=True
        )
        return False
    ...
    if not me_ok:
        await interaction.response.send_message(
            "❌ Tu n'es pas inscrit à ce tournoi. Fais `/joinleague`.",
            ephemeral=True,
        )
        return False
    if not opp_ok:
        await interaction.response.send_message(
            "❌ Ton adversaire n'est pas inscrit à ce tournoi.", ephemeral=True
        )
        return False
...
class ConfirmMatchView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.opponent_id:
            await interaction.response.send_message(
                "❌ Seul l’adversaire peut confirmer/refuser.", ephemeral=True
            )
            return False
        return True
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not p:
            ...
            return await interaction.response.edit_message(
                content="ℹ️ Ce match n’est plus en attente.", view=self
            )
        ...
        if not league or league["status"] != "open":
            ...
            self.disable_all_items()
            return await interaction.response.edit_message(
                content="❌ Tournoi fermé : confirmation impossible.", view=self
            )

        reporter_id = int(p["reporter_id"])
        opponent_id = int(p["opponent_id"])
        if not await is_player(p["league_id"], str(reporter_id)) or not await is_player(
            p["league_id"], str(opponent_id)
        ):
            await db_exec(bot.db, "DELETE FROM pending_matches WHERE id=?", (self.pending_id,))
            self.disable_all_items()
            return await interaction.response.edit_message(
                content="❌ Un joueur n’est plus inscrit : match annulé.", view=self
            )
...
async def league_create(interaction: discord.Interaction, name: str, format: str):
    ...
    if fmt not in FORMATS:
        return await interaction.response.send_message(
            "❌ Format invalide (Genesys / Compétitif / Chill).", ephemeral=True
        )
...
async def league_close(interaction: discord.Interaction, format: str):
    ...
    if not league:
        return await interaction.response.send_message(
            "❌ Aucun tournoi ouvert pour ce format.", ephemeral=True
        )
...
async def league_status(interaction: discord.Interaction, format: str):
    ...
    decks_count = await db_one(
        bot.db,
        "SELECT COUNT(*) AS c FROM decks WHERE guild_id=? AND format=?",
...
async def leaveleague(interaction: discord.Interaction, format: str):
    ...
    await interaction.response.send_message(
        f"🚪 Tu as quitté le tournoi **{fmt}**. Tes stats restent en base."
    )
...
async def deck_add(interaction: discord.Interaction, format: str, name: str):
    ...
    if len(name) > 50:
        return await interaction.response.send_message(
            "❌ Nom trop long (max 50 caractères).", ephemeral=True
        )
...
async def deck_list(interaction: discord.Interaction, format: str):
    ...
    if not rows:
        return await interaction.response.send_message(
            f"ℹ️ Aucun deck dans le répertoire **{fmt}** pour l’instant."
        )
...
# =========================
# Commands: reportmatch confirmation
# =========================
...
# =========================
# Extra: list opens + cleanup commands
# =========================
